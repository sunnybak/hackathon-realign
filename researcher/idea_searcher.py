    ...
    async def expand(self) -> List["Idea"]:
        ...
        oai_call = await async_openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant. Please provide 3 similar business ideas based on what the user says. The search criteria we are interested in is: "
                    + str(shared_state.get_search_criteria()),
                },
                {
                    "role": "user",
        ...
        for idea in ideas:
            ...
            expanded_ideas.append(
                Idea(expanded_description, self.search_criteria, parent=self)
            )
    ...
    async def expand_requirements(self):
        ...
        oai_call = await async_openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "For this business goal, create a list of 3 high level things we need to make it happen. Be more descriptive and build upon any existing requirements.",
                },
                {
                    "role": "user",
                    "content": f"Here's the goal: {goal}\nExisting requirements: {self.requirements}",
                },
            ],
...
class IdeaSearcher:
    def __init__(
        self, search_criteria: str, acceptance_criteria: dict, shared_state: SharedState
    ):
        ...
        self.requirement_expansion_depth = (
            1  # Depth at which to start expanding requirements
        )
        self.expansion_priority_penalty = 1.0  # Priority penalty for expanded ideas
        self.requirement_priority_penalty = (
            0.1  # Priority penalty for requirement expansion
        )
    ...
    async def process_queue(self):
        ...
        while self.priority_queue:
            if self.shared_state.get_search_criteria() != self.search_criteria:
                await self.update_search_criteria(
                    self.shared_state.get_search_criteria()
                )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    "http://localhost:9000/processed_ideas",
                    json={"processed_ideas": processed_ideas_json},
                ) as response:
                    if response.status == 200:
                        print("Successfully sent processed ideas to the admin.")
                    else:
                        print(
                            f"Failed to send processed ideas. Status code: {response.status}"
                        )
    ...
    async def process_single_idea(self, prioritized_item):
        ...
        print(
            f"Heuristics:\tSearch: {search_score}\tViability: {viability_score}\tCombined: {combined_score}"
        )
        ...
        self.processed_ideas.append(
            (idea, {"search_score": search_score, "viability_score": viability_score})
        )
        ...
        if parent_count >= self.depth_limit:
            ...
            jitter = random.uniform(
                -self.priority_jitter_range / 2, self.priority_jitter_range / 2
            )
            ...
            jitter = random.uniform(
                -self.priority_jitter_range, self.priority_jitter_range
            )
    ...
    async def evaluate_search_heuristic(self, idea: Idea) -> float:
        ...
        oai_call = await async_openai.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Search Criteria: {self.search_criteria}\n\nIdea: {idea.idea_description}",
                },
            ],
    ...
    async def search(self):
        async with self.lock:
            start_processing_task = asyncio.create_task(
                self.idea_researcher.start_processing()
            )
        ...
        for persona in personas:
            oai_call = await async_openai.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful assistant. Please generate 3 business ideas for the given persona provided the search criteria. The search criteria we are interested in is: "
                        + str(self.search_criteria),
                    },
                    {
                        "role": "user",
                        "content": "Here's the prospective persona: "
                        + persona
            ...
            for idea in ideas:
                expanded_description = idea["idea_description"]
                expanded_ideas.append(
                    Idea(expanded_description, self.search_criteria, parent=None)
                )
        return expanded_ideas
...
async def main(shared_state):
    ...
    for idea, criteria in accepted:
        print(
            f"- {idea.idea_description} (Search Score: {criteria['search_score']}, Viability Score: {criteria['viability_score']})"
        )
        print("Idea lineage:")
        idea.print_lineage("  ")
